/* "Highest number of non-null values for a given set of columns" --
   now count/filled, one of the two counting questions. */
...
test("scores the count of non-empty cells in the chosen columns", () => {
  const rule = R.makeCountRule(cellAt, ROWS.length, COLS, { wsBlank: true, counts:"filled" });
...
test("whitespace-only cells count as filled when the setting is off", () => {
  const rule = R.makeCountRule(cellAt, ROWS.length, COLS, { wsBlank: false, counts:"filled" });
...
test("direction can be flipped to keep the least complete row", () => {
  const rule = R.makeCountRule(cellAt, ROWS.length, COLS, { wsBlank: true, dir: -1, counts:"filled" });
...
test("only the chosen columns are counted", () => {
  const rule = R.makeCountRule(cellAt, ROWS.length, [2, 3], { wsBlank: true, counts:"filled" });
...
test("reports its type and column set", () => {
  const rule = R.makeCountRule(cellAt, ROWS.length, [1, 2], { wsBlank: true, counts:"filled" });
  assert.equal(rule.type, "count");
